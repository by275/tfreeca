...
class ModuleMain(PluginModuleBase):
    db_default = {
        "site_url": "",
        "http_proxy": "",
        "download_program": "transmission",
        "download_path": "",
    }

    def __init__(self, PM):
        super().__init__(PM, name="main")
        self.session = None
    ...
    def process_menu(self, sub, req):
        ...
        if sub == "down" and req.method == "GET":
            try:
                fcontent, filename = self.tf_down(
                    req.query_string.decode("utf-8"), item_no=int(req.args.get("item_no", "0"))
                )
        ...
        try:
            ...
            if sub == "get_torrent_info":
                ...
                if href:
                    ...
                    mod = F.PluginManager.get_plugin_instance("torrent_info").module_list[0]
                    return jsonify({"success": True, "info": mod.parse_torrent_file(fcontent)})
            if sub == "get_more":
                href = p.get("href", "")
                if href:
                    src_url = ModelSetting.get("site_url").rstrip("/") + "/board.php?" + href.split("?")[1]
